...
void Custom::RobotControl()
{
    ...
    if(show == 1){
        ...
        if (show_count%100 == 0){ 
            // printf("r2:%i",rc_command.right_lower_right_switch);

            // 打印关节位置
            printf("joint_state_q:[");
            for(int i = 0; i < 12; i++){
                printf(" %f,",joint_state_simple.q[i]);
            }
            printf("]\n");
            // 打印关节力矩
            printf("joint_state_tau_est:[");
            for(int i = 0; i < 12; i++){
                printf(" %f,",joint_state_simple.tau_est[i]);
            }
            printf("]\n");

            // 打印关节位置命令
            printf("joint_command_q_des:[");
            for(int i = 0; i < 12; i++){
                printf(" %f,",joint_command_simple.q_des[i]);
            }
            printf("]\n");
            // 打印关节位置命令
            printf("joint_command_tau_ff:[");
            for(int i = 0; i < 12; i++){
                printf(" %f,",joint_command_simple.tau_ff[i]);
            }
            printf("]\n");
            printf("\n");
        }
